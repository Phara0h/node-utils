...
class PGActiveModel extends BaseModel {
    ...
    async find() {
        var foundUser = await this.ChildClass.findById(this[this._pkKey]);

        if (!foundUser) {
            throw new Error('Unable to find model with id ' + this[this.pkKey]);
        } else {
            this._setModel(foundUser);
        }
        return this;
    }
    ...
    async delete() {
        ...
        var deletedUser = await this.ChildClass.deleteById(this[this._pkKey]);

        if (deletedUser.length < 1) {
            throw new Error('Unable to delete model with id ' + this[this._pkKey]);
        } else {
            this._setUserPropsToNull();
        }
    ...
    async create() {
        ...
        var newUser = await this.ChildClass.create(this.changedProps, true);

        if (!newUser) {
            throw new Error('Unable to create new model with id ' + this._[this._pkKey]);
        } else {
            this._setModel(newUser);
        }

        return this;
    }
    static async create(model, nonStatic = false) {
        var newUser = await super.create(model);

        if (!newUser) {
            throw new Error('Unable to create new model');
        } else {
            newUser = nonStatic ? newUser[0] : new this(newUser[0]);
        }
    ...
    async update(model) {
        var updatedUser = await this.ChildClass.updateById(this[this._pkKey], model, true, false);

        if (!updatedUser) {
            throw new Error('Unable to update model with id ' + this[this._pkKey]);
        } else {
            this._setModel(updatedUser);
        }
    }
