...
class PGEncryptModel extends BaseModel {
    ...
    static async findAllBy(fieldValues, operator = 'AND', limit) {
        return super.findAllBy(await this._queryFieldsHash(fieldValues, this.getEncryptedProfile(fieldValues)), operator, limit).then(async newModels=>{
    ...
    static async updateAllBy(fieldValues, model, operator = 'AND', returnModel = true, encryptProfile, limit) {
        return super.updateAllBy(await this._queryFieldsHash(fieldValues, encryptProfile), await this.encrypt(model, encryptProfile), operator, returnModel, limit).then(async newModels=>{
          for (var i = 0; i < newModels.length; i++) {
            if(newModels[i]) {
                newModels[i] = await this.decrypt(newModels[i], this.getEncryptedProfile(newModels[i]), true)
            }
