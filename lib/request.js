...
var request = function request(options) {
    ...
    function promiseRequest(resolve, reject, count = 0) {
        ...
        var req = client[proto].request(options.uri, options, (res) => {
            ...
            res.on('end', () => {
                ...
                if (REDIRECT_CODES.indexOf(res.statusCode) !== -1 && count < options.redirectMax) {
                    options.uri = url.resolve(options.uri, res.headers.location);
                    options.proto = options.uri.split(':')[0];
                    return promiseRequest(resolve, reject, ++count);
                } else {
        ...
        req.on('error', async (e) => {
            ...
            if (e.code == 'ECONNRESET' && options.retryReset) {
                options.retryReset = false;
                try {
